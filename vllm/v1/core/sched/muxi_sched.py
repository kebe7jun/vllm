...
class Scheduler(SchedulerInterface):
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        skip_sched_wait = False
        if self.waiting:
            if len(self.decode) + len(self.running) >= 100:
                skip_sched_wait = True
                self.running.extend(self.decode)
                self.decode.clear()
            else:
                self.decode.extend(self.running)
                self.running.clear()
                skip_sched_wait = False
        else:
            self.running.extend(self.decode)
        ...
        if not preempted_reqs and not skip_sched_wait:
            while (
                self.waiting
                and token_budget > 0
                and len(self.decode) + len(self.running) < 100
            ):
